...
def compute_power(x, fs, pim_sft, pim_bw, return_db=False):
    n = 2048
    f, pxx = welch(x, fs, window=np.kaiser(2048, 10), nperseg=n, noverlap=1, nfft=n)
    freq_mask = np.where((f > pim_sft - pim_bw / 2) & (f < pim_sft + pim_bw / 2))
    # freq_mask = np.where((f >  - 5 / 2 - 27.5) & (f < 5 / 2 - 27.5)) # data_A
    # freq_mask = np.where((f >  - 5 / 2 + 15) & (f < 5 / 2 + 15)) # data_B

    pxx_window = pxx[freq_mask[0]]
...
def compute_db_power(signal, FS, FC_TX=0, PIM_SFT=0, PIM_BW=0):
    ...
    rng = np.where(
        (f > FC_TX + PIM_SFT - PIM_BW / 2) & (f < FC_TX + PIM_SFT + PIM_BW / 2)
    )
...
def compute_perf_db_input(x_pwr_db, y_pwr_db):
    perf = 10 * np.log10(10 ** (x_pwr_db / 10) - 1) - 10 * np.log10(
        10 ** (y_pwr_db / 10) - 1
    )
    return perf
