...
class ComplexScaling(nn.Module):
    ...
    def forward(self, x):
        ...
        if ndim == 3:  # (batch_size, n_channels, 2)
            ...
            w_real = self.weights[:, 0].unsqueeze(0).unsqueeze(0)  # shape: (1, 1, n_channels)
            w_imag = self.weights[:, 1].unsqueeze(0).unsqueeze(0)  # shape: (1, 1, n_channels)
        ...
        if ndim == 3:
            ...
            new_real = new_real.unsqueeze(-1)  # shape: (batch_size, seq_len, n_channels, 1)
            new_imag = new_imag.unsqueeze(-1)  # shape: (batch_size, seq_len, n_channels, 1)
...
class TxaFilterEnsemble(nn.Module):
    ...
    def forward(self, x):
        ...
        output = torch.empty(
            (n_batch, self.out_seq_size, self.n_channels, 2),
            device=x.device
        )
