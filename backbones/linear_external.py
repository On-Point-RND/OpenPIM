...
class Linear(nn.Module):
    ...
    def forward(self, x, h_0=None):
        ...
        x_init_real = x[:, :, :, 0]  # (B, N, C)
        x_init_imag = x[:, :, :, 1]  # (B, N, C)
        ...
        for c, filt_layer in enumerate(self.filter_layers_in):
            ...
            for id in range(self.out_window):
                ...
                filtered_real[:, id, c] = f_real.squeeze(-1)
        ...
        for id in range(self.out_window):
            ...
            for c, nonlin_layer in enumerate(self.nonlin_layers):
                ...
                c_real = c_output[:, 0]
                c_imag = c_output[:, 1]
                amp = c_real.pow(2) + c_imag.pow(2)
                c_real = self.amp1_weight * amp * c_real
                c_imag = self.amp2_weight * amp * c_imag

                nonlin_real[:, id, c] = c_real.squeeze(-1)
