...
class Gate(nn.Module):
    ...
    def forward(self, x, return_aux_loss=False):
        ...
        gate_out = self.gate_activation(gate_out)
        if return_aux_loss:
...
class MoeNonlinearity(nn.Module):
    def __init__(self, seq_size, n_channels, add_expert, return_aux_loss=True, aux_loss_weight=1e-6):
        super().__init__()
        self.n_channels = n_channels
        self.seq_size = seq_size
        self.return_aux_loss = return_aux_loss
        self.aux_loss_weight = aux_loss_weight
        # options: relu, tanh, elu, silu, none, gelu, selu, softplus
        experts_list = [
            "silu",
        ]
        experts_list += [add_expert]
...
class PairedMoE(nn.Module):
    def __init__(self, in_seq_size, out_seq_size, n_channels, add_expert,
